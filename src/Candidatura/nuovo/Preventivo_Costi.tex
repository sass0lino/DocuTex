...
% blocco di codice che permette di includere immagini con \includegraphics{logo.jpg} 
% e i file ausiliari con \pjinput{\contenuti/file.tex} senza preoccupazioni
% --- Gestione percorsi dinamici per progetti LaTeX ---
\usepackage{currfile}
\usepackage{graphicx}
\usepackage{xstring}

% Ottieni il percorso assoluto del file principale (es: /home/.../src/.../progetto/)
\edef\pjrootfull{\currfileabsdir}

% Estrai il percorso relativo a partire da "src/"
\StrBehind{\pjrootfull}{src/}[\pjsubpath]

% Ricostruisci il percorso del progetto, relativo alla root del repository
\edef\pjroot{src/\pjsubpath}

% Directory in cui cercare le immagini (ordine di ricerca)
\graphicspath{
  {\pjroot}              % cartella del progetto
  {\pjroot/immagini/}    % immagini specifiche del progetto
  {src/immagini/}        % immagini condivise tra progetti
  {assets/immagini/}     % immagini globali (se presenti)
}

% Macro per includere i file locali al progetto
\newcommand{\pjinput}[1]{\input{\pjroot#1}}
...
\pjinput{contenuti/intestazione_titolo.tex}
